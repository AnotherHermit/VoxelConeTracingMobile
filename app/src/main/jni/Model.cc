...
void Model::SetStandardData(size_t numVertices, GLfloat *verticeData,
	...
	GL_CHECK(glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * numVertices,
												verticeData, GL_STATIC_DRAW));

	GL_CHECK(glBindBuffer(GL_ARRAY_BUFFER, meshBuffers[1]));
	GL_CHECK(glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * numNormals,
												normalData, GL_STATIC_DRAW));

	GL_CHECK(glBindBuffer(GL_ARRAY_BUFFER, meshBuffers[2]));
	GL_CHECK(glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * numTangents,
												tangentData, GL_STATIC_DRAW));

	GL_CHECK(glBindBuffer(GL_ARRAY_BUFFER, meshBuffers[3]));
	GL_CHECK(glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * numBiTangents,
												biTangentData, GL_STATIC_DRAW));

	GL_CHECK(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, meshBuffers[4]));
	GL_CHECK(glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(GLuint) * numIndices,
...
void Model::SetTextureData(size_t numTexCoords, GLfloat *texCoordData) {
	...
	GL_CHECK(glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * numTexCoords,
...
void Model::Voxelize() {
	...
	GL_CHECK(glDrawElements(GL_TRIANGLES, (GLsizei) nIndices, GL_UNSIGNED_INT,
													0L));
}

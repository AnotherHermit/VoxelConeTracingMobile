...
bool ModelLoader::LoadTextures() {
    ...
    for (size_t i = 0; i < materials.size(); i++) {
        ...
        data->diffColor = glm::vec3(materials[i].diffuse[0],
...
GLuint ModelLoader::LoadTexture(const char *path) {
    ...
    GL_CHECK(glTexImage2D(GL_TEXTURE_2D, 0, type, width, height, 0, type,
...
bool ModelLoader::CalculateMinMax(glm::vec3 **maxVertex,
                                  glm::vec3 **minVertex) {
    ...
    for (auto shape = shapes.begin(); shape != shapes.end(); shape++) {
        ...
        for (auto vertex = shape->mesh.positions.begin();
            ...
            glm::vec3 currentVertex = glm::vec3(vertex[0], vertex[1],
                                                vertex[2]);
...
void ModelLoader::CalculateTangents() {
    for (auto shape = shapes.begin(); shape != shapes.end(); shape++) {
        ...
        for (auto normal = shape->mesh.normals.begin();
            ...
            glm::vec3 currentNormal = glm::normalize(
